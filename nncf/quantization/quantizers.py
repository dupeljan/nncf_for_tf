...
class SymmetricQuantizer(Quantizer):
    def __init__(self, config):
        ...
        self.signed = config.signed \
            if config.signed is None else True
        self._signedness_to_force = config.signed
    ...
    def apply_minmax_initialization(self, weights, min_values, max_values, min_range=0.1, eps=0.01):
        if self._signedness_to_force is None:
            self.signed = bool(tf.reduce_any(tf.less(min_values, 0)))
    ...
    def __eq__(self, other):
        if not isinstance(other, SymmetricQuantizer):
            return False
        return (self.num_bits == other.num_bits and
                self.per_channel == other.per_channel and
                self.narrow_range == other.narrow_range and
...
class AsymmetricQuantizer(Quantizer):
    ...
    def __eq__(self, other):
        if not isinstance(other, AsymmetricQuantizer):
            return False
        return (self.num_bits == other.num_bits and
                self.per_channel == other.per_channel and
